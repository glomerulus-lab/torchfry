...
This script runs experiments for training MLP models on Fashion MNIST using Fastfood or RKS projection layers.
...
def parse_all_args():
    ...
    argparse.Namespace
        ...
        - save: Name of the file to save results
    """
...
for key in configs:
    ...
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=config["mb"], num_workers=0, shuffle=True)
    testloader = torch.utils.data.DataLoader(testset, batch_size=config["mb"], num_workers=0, shuffle=False)
